...
// Devuelve un número o null si no pudo reconocer el patrón. Es solo una
// ayuda visual en la ficha del producto — no reemplaza cargar datos
// reales en la planilla si se necesita precisión.
function extraerUnidadesPorBulto(nombre, presentacion) {
const texto = `${nombre || ""} ${presentacion || ""}`;
let m = texto.match(/(\d+)\s*uni\.?\s*x/i);
if (m) return parseInt(m[1], 10);
m = texto.match(/x\s*(\d+)\s*uni/i);
if (m) return parseInt(m[1], 10);
...
const precioBultoTexto = iPrecioBulto >= 0 ? f[iPrecioBulto] : "";
const precioBulto = precioBultoTexto ? parseMonedaAR(precioBultoTexto) : null;
const sku = (f[iSKU] || "").trim();
const proveedor = (iProv >= 0 && f[iProv]) ? f[iProv].trim() : nombreProveedor;
const nombreProd = (f[iProd] || "").trim();
const presentacionProd = (f[iPres] || "").trim();
productos.push({
...
precioBulto: precioBulto && !isNaN(precioBulto) ? precioBulto : null,
unidadesPorBulto: extraerUnidadesPorBulto(nombreProd, presentacionProd),
imagen: (f[iImg] || "").trim(),
...
const nombreProd = p.Producto || "";
const presentacionProd = p.Presentacion || "";
return {
...
precioBulto: p.PrecioBulto ? Number(p.PrecioBulto) : null,
unidadesPorBulto: extraerUnidadesPorBulto(nombreProd, presentacionProd),
imagen: p.Imagen || "",
